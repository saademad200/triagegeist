...
def prepare_data():
    print("Loading data for PyTorch MLP...")
    ...
    if len(cat_cols) > 0:
        ...
        # Shift to 0-index for unknown, so classes start at 1
        df[cat_cols] = df[cat_cols] + 1
        
        for col in cat_cols:
            num_unique = int(df[col].max()) + 1
            cat_dims.append(num_unique)
            
    X_num = df[num_cols].values
    X_cat = df[cat_cols].values.astype(int) if len(cat_cols) > 0 else None
    
    return X_num, X_cat, y, cat_dims
...
def main():
    set_seed(SEED)
    X_num, X_cat, y, cat_dims = prepare_data()
    ...
    for fold, (train_idx, valid_idx) in enumerate(skf.split(X_num, y)):
        print(f"Fold {fold+1}")
        X_num_tr, X_cat_tr, y_tr = X_num[train_idx], X_cat[train_idx], y[train_idx]
        X_num_vl, X_cat_vl, y_vl = X_num[valid_idx], X_cat[valid_idx], y[valid_idx]
        ...
        model = TabularMLP(X_num.shape[1], cat_dims).to(device)
    ...
    print(f"Overall QWK (PyTorch MLP): {overall_qwk:.4f}")
    
    os.makedirs('results/exp0024_track1_pytorch_mlp', exist_ok=True)
    np.save('results/exp0024_track1_pytorch_mlp/oof_preds.npy', oof_preds)
    
    with open('results/exp0024_track1_pytorch_mlp/metrics.json', 'w') as f:
